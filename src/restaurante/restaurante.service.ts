...
export class RestauranteService {
  ...
  async findAll(): Promise<RestauranteEntity[]> {
    return await this.restauranteRepository.find({ relations: ["culturasGastronomicas"] });
  }

  async findOne(id: string): Promise<RestauranteEntity> {
    try {
      const restaurante: RestauranteEntity = await this.restauranteRepository.findOne({ where: { id }, relations: ["culturasGastronomicas"] });
      if (!restaurante)
